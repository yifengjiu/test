...
public class CalendarUtils {
    ...
    private static String CALENDAR_ERR="XXXXX";
    private static String EVENT_ERROR="XXXXX";
    private static String REMIND_ERROR="XXXXX";
    ...
    public static void addCalendarEventRemind(Context context, @NonNull String title, String description, long begintime, long endtime, int remind_minutes, onCalendarRemindListener callback){
        ...
        if(calendar_id < 0){
            // 获取日历失败直接返回
            if(null != callback){
                callback.onFailed(onCalendarRemindListener.Status._CALENDAR_ERROR);//onCalendarRemindListener.Status.CALENDAR_ERR);
            }
            return;
        }
